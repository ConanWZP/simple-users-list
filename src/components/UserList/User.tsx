...
const User: FC<UserType> = ({user, onClickInvite, invitedUsers}) => {

    let isInvited;

    if (invitedUsers.find(userId => userId === user.id)) {
        isInvited = true
    } else {
        isInvited = false
    }
    ...
    return (
        <li>
            ...
            <img onClick={() => onClickInvite(user.id)} className="action" src={isInvited ? minus : plus} alt="Action"/>
        </li>
    )
};
